...
contract WaterfallPresale is Ownable {
	...
	function processPurchase(uint256 etherValue, address user) internal {
		require(block.timestamp >= presaleStart, "processPurchase: presale has not yet started");
		require(block.timestamp <= presaleEnd, "processPurchase: presale has ended");
	...
	function etherWaterfall(uint256 etherValue) internal {
		...
		while(etherRemaining >= ethToTarget){
			...
			uniswapPoolEth += ( ethToTarget.mul(percentLevels[currentEthTarget]) ).div(percentPrecision);
			etherRemaining -= ethToTarget;
			currentEthTarget += 1;
			//get ETH size for next level of waterfall
			ethToTarget = ethTargets[currentEthTarget] - ethTargets[(currentEthTarget - 1)];
		}
		...
		uniswapPoolEth += ( etherRemaining.mul(percentLevels[currentEthTarget]) ).div(percentPrecision);
	...
	function getTokensFromEth(uint256 ethInput, address user) internal {
		uint256 tokensToCeiling = tokenCeilings[currentPriceLevel] - totalTokensSold;
		...
		while(valueRemaining.mul(tokensPerEth[currentPriceLevel]).div(uint256(1e18)) >= tokensToCeiling){
			//move to next level of LP waterfall
			require(currentPriceLevel < (tokenCeilings.length - 1), "getTokensFromEth: purchase would exceed max token ceiling");
			tokens += tokensToCeiling;
			valueRemaining -= tokensToCeiling.mul(uint256(1e18)).div(tokensPerEth[currentPriceLevel]);
			currentPriceLevel += 1;
			//get token size for next level of waterfall
			tokensToCeiling = tokenCeilings[currentPriceLevel] - tokenCeilings[(currentPriceLevel - 1)];
		}
		...
		tokens += valueRemaining.mul(tokensPerEth[currentPriceLevel]).div(uint256(1e18));
	...
	function claimTokens() external returns(uint256) {
		require( (block.timestamp >= claimStart || endedEarly), "claimTokens: tokens cannot yet be claimed" );
	...
	function recoverUnclaimed() external onlyOwner() {
		require(block.timestamp >= claimEnd, "recoverUnclaimed: token claim period has not yet ended");
	...
	function recoverTokens(address tokenAddress, address dest, uint256 amountTokens) external onlyOwner() {
		require(tokenAddress != quaiToken, "recoverTokens: cannot move sold token");
		IERC20(tokenAddress).transfer(dest, amountTokens);
	}

	//gets expected amount of ETH from input amount of ERC20 token
	function getExpectedEth(address tokenAddress, uint256 amountIn) public view returns(uint256) {
	...
	function newPurchaseOption(address tokenAddress) external onlyOwner() {
		require(approvedTokens[tokenAddress] == false, "newPurchaseOption: token already added");
	...
	function purchaseWithERC20(address tokenAddress, uint256 amountIn) external {
		require(approvedTokens[tokenAddress] == true, "token is not approved as payment option");
		require(IERC20(tokenAddress).allowance(msg.sender, address(this)) >= amountIn, "token approval is insufficent for purchase");
        ...
        uint256[] memory _amts = router.swapExactTokensForETH(amountIn, uint(0), path, address(this), deadline);
        processPurchase(_amts[1], msg.sender);
	}

	function expectedTokensFromETH(uint256 ethInput) public view returns(uint256){
		uint256 tokensToCeiling = tokenCeilings[currentPriceLevel] - totalTokensSold;
		uint256 valueRemaining = ethInput;
		uint256 tokens = 0;
		...
		//token waterfall -- while loop covers all cases where purchase value overflows one or more token levels
		while(valueRemaining.mul(tokensPerEth[virtualPriceLevel]).div(uint256(1e18)) >= tokensToCeiling){
			require(virtualPriceLevel < (tokenCeilings.length - 1), "getTokensFromEth: purchase would exceed max token ceiling");
			tokens += tokensToCeiling;
			valueRemaining -= tokensToCeiling.mul(uint256(1e18)).div(tokensPerEth[virtualPriceLevel]);
			//move to next level of waterfall
			virtualPriceLevel += 1;
			//get token size for next level of waterfall
			tokensToCeiling = tokenCeilings[virtualPriceLevel] - tokenCeilings[(virtualPriceLevel - 1)];
		}

		//get tokens for current waterfall level -- this is the only part of the logic that will be called when the purchase does not overflow a level
		tokens += valueRemaining.mul(tokensPerEth[virtualPriceLevel]).div(uint256(1e18));
		return tokens;
	}

	function expectedTokensFromERC20(address tokenAddress, uint256 amountIn) external view returns(uint256){
...
contract QUAIDAO_Staking is Ownable {
	...
	function unstake() external {
		...
		require(timeSinceStake >= stakingPeriod, "unstake: staking period for user still ongoing");
		updateStake();
	...
	function recoverQUAI() external onlyOwner() {
		require(block.timestamp >= (stakingEnd + 60 days), "recoverQUAI: too early");
		uint256 amountToSend = IERC20(quaiToken).balanceOf(address(this));
		IERC20(quaiToken).transfer(msg.sender, amountToSend);
	}
